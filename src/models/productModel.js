...
const product = {
   getAll: async () => {
      let response = await connection.query('select id, name, price, description, image_link from "Products" where type = 1') //Pizzas
      const pizzas = response.rows

      response = await connection.query('select id, name, price, description, image_link from "Products" where type = 2') //Pizza2Sabores
      const twoFlavorPizza = response.rows

      response = await connection.query('select id, name, price, description, image_link from "Products" where type = 3') //Combos
      const combos = response.rows

      response = await connection.query('select id, name, price, description, image_link from "Products" where type = 4') //Drinks
